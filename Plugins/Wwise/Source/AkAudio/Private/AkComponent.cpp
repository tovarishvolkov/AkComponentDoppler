...
void UAkComponent::DopplerEffect(float DeltaTime)
{
	if (bDopplerEffect)
	{
		auto PController = GetWorld()->GetFirstPlayerController();
		FVector SourceCurrentPosition = GetPosition();

		if (!IsListener && PController != nullptr)
		{
			FVector ListenerPosition = PController->GetPawn()->GetActorLocation() + FVector(0.0f, 0.0f, HeightOffset);
			FVector ListenerVelocity = PController->GetPawn()->GetVelocity();

			if (SourcePrevPosition.IsZero())
			{
				SourcePrevPosition = SourceCurrentPosition;
			}

			FVector SourceVelocity = (SourceCurrentPosition - SourcePrevPosition) / DeltaTime;

			FVector DirectionToListener = (ListenerPosition - SourceCurrentPosition);

			if (!bSpeedDotProduct)
			{
				float ListenerSpeed = DirectionToListener.Length() - (ListenerVelocity.ProjectOnTo(DirectionToListener) + DirectionToListener).Length();
				float SourceSpeed = (SourceVelocity.ProjectOnTo(DirectionToListener) + DirectionToListener).Length() - DirectionToListener.Length();

				DopplerFactor = (SpeedOfSound + ListenerSpeed) / (SpeedOfSound - SourceSpeed) - 1.0f;
			}

			if (bSpeedDotProduct)
			{
				float ListenerSpeedDot = FVector::DotProduct(ListenerVelocity, DirectionToListener.GetSafeNormal());
				float SourceSpeedDot = FVector::DotProduct(SourceVelocity, DirectionToListener.GetSafeNormal());

				DopplerFactor = (SpeedOfSound + ListenerSpeedDot) / (SpeedOfSound - SourceSpeedDot) - 1.0f;
			}

			SourcePrevPosition = SourceCurrentPosition;

			if (DopplerRTPC) SetRTPCValue(DopplerRTPC, DopplerFactor, 0, "");

			if (GEngine && bDrawDebugDoppler)
			{
				GEngine->AddOnScreenDebugMessage(-1, 0.0f, FColor::Red, FString::Printf(TEXT("Doppler: %f"), DopplerFactor));
				DrawDebugLine(GetWorld(), SourceCurrentPosition, ListenerPosition, FColor::Red, false, 0.1f, 0, 1.0f);
			}
		}
	}
}
...
void UAkComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	if (SoundEngine->IsInitialized())
	{
		...
		if (World) DopplerEffect(DeltaTime);
